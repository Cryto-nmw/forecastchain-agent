...
export const createForecastGame = async (
  ...
  const FORECAST_GAME_FACTORY_ABI = await JSON.parse(
    FORECAST_GAME_FACTORY_ABI_str
  );
  ...
  try {
    ...
    try {
      ...
      const errorMessage = `###Detailed error: ${
        error.message
      } ${JSON.stringify(FORECAST_GAME_FACTORY_ABI)} <--`;
    ...
    const valueInWeiV6 = ethers.parseEther(fundPool);

    // console.log(`create game with wei ${valueInWeiV6}`);
    // return false;
